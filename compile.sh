...
if [ "$#" -ne 2 ]; then
    ...
    echo "[*]        Enter path to: <outputfile>  Enter path to: <src file> Enter "
...
$TOOLCHAIN -g -fPIC  -pie -Os -Wall -fvisibility=hidden -o lib/libpaho-mqtt3cs.so.1.0 src/MQTTPersistence.c src/Heap.c src/Socket.c src/SSLSocket.c src/MQTTPacket.c src/Clients.c src/MQTTClient.c src/Log.c src/MQTTPacketOut.c src/MQTTProtocolOut.c src/StackTrace.c src/MQTTPersistenceDefault.c src/utf-8.c src/Messages.c src/SocketBuffer.c src/MQTTProtocolClient.c src/LinkedList.c src/Thread.c src/Tree.c   -shared -Wl,--start-group  -ldl -Wl,--end-group -Wl,-init,MQTTClient_init -Wl,-soname,libpaho-mqtt3cs.so.1 -Wl,-no-whole-archive
